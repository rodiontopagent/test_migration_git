...
class SpUpdateOrderPayments extends Migration {
	...
	{
		$sql = <<<SQL
      CREATE PROCEDURE sp_updateOrderPayments (IN i_OrderId INT)
...
BEGIN
	...
	INTO i_currency_id, i_currency_rate, i_project_currency_id, i_project_currency_rate, i_amount, i_amount_net;
		...
		ELSE
			...
			SET @total_paid_net = @total_paid_net + i_amount_net * i_currency_rate / (i_project_currency_rate - 0.14);
		END IF;
	END LOOP the_loop;
		
	UPDATE `order` SET `order`.paid_gross = @total_paid, `order`.paid_net = @total_paid_net WHERE `order`.id = i_OrderId;
...
SQL;
	...
	{
        DB::unprepared('DROP PROCEDURE IF EXISTS sp_updateOrderPayments');
	}
