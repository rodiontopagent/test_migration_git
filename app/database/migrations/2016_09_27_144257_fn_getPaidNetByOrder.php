...
class FnGetPaidNetByOrder extends Migration {
	...
	{
		$sql = <<<SQL
    CREATE FUNCTION fn_getPaidNetByOrder (i_OrderId MEDIUMINT) RETURNS DECIMAL
...
BEGIN
	...
	INTO i_currency_id, i_currency_rate, i_project_currency_id, i_project_currency_rate, i_amount, i_amount_net;
		...
		IF(i_currency_id = i_project_currency_id) THEN
			SET @total_paid = @total_paid + i_amount_net;
		ELSEIF((i_currency_id = 1 AND i_project_currency_id != 1) OR (i_currency_id != 1 AND i_project_currency_id = 1)) THEN
			SET @total_paid = @total_paid + i_amount_net * i_currency_rate / i_project_currency_rate;
		ELSE
		    SET @total_paid = @total_paid + i_amount_net * i_currency_rate / (i_project_currency_rate - 0.14);
		END IF;
	END LOOP the_loop;
	RETURN @total_paid;
END
SQL;
	...
	{
        DB::unprepared('DROP FUNCTION IF EXISTS fn_getPaidNetByOrder');
	}
